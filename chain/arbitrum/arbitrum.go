...
func NewChainAdaptor(conf *config.Config) (chain.IChainAdaptor, error) {
	ethClient, err := evmbase.DialEthClient(context.Background(), conf.WalletNode.Arbi.RpcUrl)
	if err != nil {
		return nil, err
	}
	ethDataClient, err := evmbase.NewEthDataClient(conf.WalletNode.Arbi.DataApiUrl, conf.WalletNode.Arbi.DataApiKey, time.Second*20)
	if err != nil {
		return nil, err
	}
	return &ChainAdaptor{
